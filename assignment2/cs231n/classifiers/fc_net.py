...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
          ...
          x, cache = affine_norm_relu_forward(w,b,x, gamma, beta, bn_params, self.use_dropout, self.normalization, self.dropout_param)  
          
          caches.append(cache)
        
        
        out, cache = affine_forward(x, self.params['W' + str(self.num_layers)], self.params['b' + str(self.num_layers)]) 
        caches.append(cache)  
        ...
        for i in range(self.num_layers - 2, -1, -1):
          dx, dw, db, dgamma, dbeta = affine_norm_relu_backward(dx, caches[i], self.normalization, self.use_dropout)
...
def affine_norm_relu_forward(w,b,A, gamma, beta, bn_params, use_dropout, normalization, dropout_params):
  bn_cache, do_cache = None, None

  out, fc_cache = affine_forward(A, w, b)
