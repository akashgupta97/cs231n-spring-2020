...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (xhat, gamma, beta, xmu, sqrtvar, ivar, axis1)
        ...
        x = x - running_mean
        x = np.divide(x, np.sqrt(running_var + eps))
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    xhat, gamma, beta, xmu, sqrtvar, ivar, axis1 = cache
    N, D = dout.shape
    ...
    dxhat = gamma*dsum1
    dgamma = np.sum(dsum1*xhat, axis=0)
...
def batchnorm_backward_alt(dout, cache):
    ...
    dgamma = np.sum(xhat * dout, axis=axis1)
    
    #Take gamma in because of broadcasting difference. gamma - (D,), ivar - (N,) -  Not allowed
    dx = (1. / N) * ivar * (N * dout * gamma - np.sum(dout*gamma, axis=0)
    - xmu * (ivar**2) * np.sum(dout *gamma* xmu, axis=0))
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    out, cache = batchnorm_forward(x.T, gamma.reshape(-1, 1), beta.reshape(-1, 1), {'mode': mode, 'axis': 1})
    out = out.T
...
def conv_backward_naive(dout, cache):
    ...
    for index in range(N):
      neuron = 0
      out_col = dout[index].reshape(F, outH*outW)        #[FxH'*W']
